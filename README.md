...
- **Volatile Memory Zeroization (`zeroize` Crate)**: Mengintegrasikan crate resmi `zeroize` dan memory fence `compiler_fence(Ordering::SeqCst)` untuk memastikan penimpaan fisik (*volatile zero overwrite*) pada kunci privat perantara, seed bytes, dan frasa pemulihan di RAM baik saat sesi dibatalkan (*cancelled*) maupun selesai (*completed*).
